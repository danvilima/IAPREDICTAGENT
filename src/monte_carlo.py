...
def simular_torneio_detalhado(df_jogos, df_grupos, df_calendario_mata):
    pontos = defaultdict(int)
    gols_pro = defaultdict(int)
    saldo_gols = defaultdict(int)
    ...
    for g, df_g in df_grupos.groupby('group'):
        ...
        for pos, dados in enumerate(dados_grupo):
            t = dados[4]
            tabela_grupos.append({
                ...
                'derrotas': derrotas[t],
                'gols_pro': gols_pro[t],
                'gols_contra': gols_contra[t],
                'saldo_gols': saldo_gols[t],
                'pontos': pontos[t]
            })
    ...
    df_res_grupos = pd.DataFrame(tabela_grupos)
    ...
    # 2. Mata-Mata
    mata_mata_res = defaultdict(list)
    podio = {}
    ...
    for row in df_calendario_mata.itertuples():
        ...
        # Identificar fase
        fase_chave = 'R32'
        if match_id.startswith('M7') or match_id.startswith('M8'):
            fase_chave = 'R32'
        elif match_id.startswith('M89') or match_id.startswith('M90') or match_id.startswith('M91') or match_id.startswith('M92') or match_id.startswith('M93') or match_id.startswith('M94') or match_id.startswith('M95') or match_id.startswith('M96'):
            fase_chave = 'R16'
        elif match_id.startswith('M97') or match_id.startswith('M98') or match_id.startswith('M99') or match_id.startswith('M100'):
            fase_chave = 'QF'
        elif match_id in ['M101', 'M102']:
            fase_chave = 'SF'
        elif match_id == 'M103':
            fase_chave = '3RD_PLACE'
            podio['terceiro'] = vencedor
        elif match_id == 'M104':
            fase_chave = 'FINAL'
            podio['campeao'] = vencedor
            podio['vice'] = perdedor
            
        mata_mata_res[fase_chave].append({
            'time_casa': t1,
            'time_visitante': t2,
            'gols_casa': g1,
            'gols_visitante': g2,
    ...
    return {
        'podio': podio,
        'mata_mata': dict(mata_mata_res),
        'grupos': df_res_grupos
    }
