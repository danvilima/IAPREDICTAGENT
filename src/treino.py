...
def main():
    ...
    assert (
        df[COLUNAS_OBRIGATORIAS].isna().sum().sum() == 0
    ), "Falha: Dados nulos detectados no dataframe."
    assert (
        df["peso_amostra"] > 0
    ).all(), "Falha: Existência de peso_amostra nulo ou negativo."
    ...
    modelo_casa_validacao = sm.GLM(
        y_casa_treino, X_treino, family=sm.families.Poisson(), freq_weights=peso_treino
    ).fit()

    modelo_visitante_validacao = sm.GLM(
        y_visit_treino, X_treino, family=sm.families.Poisson(), freq_weights=peso_treino
    ).fit()
    ...
    for lc, lv, gc, gv in zip(
        lambda_casa_teste, lambda_visitante_teste, y_casa_teste, y_visit_teste
    ):
    ...
    with open("models/modelo_poisson_casa.pkl", "wb") as f:
        pickle.dump(modelo_casa_final, f, protocol=4)

    with open("models/modelo_poisson_visitante.pkl", "wb") as f:
        pickle.dump(modelo_visitante_final, f, protocol=4)

    with open("models/colunas_atributos.pkl", "wb") as f:
        pickle.dump(COLUNAS_ATRIBUTOS, f, protocol=4)
